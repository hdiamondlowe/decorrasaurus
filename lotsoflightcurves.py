...
#directoryname = 'joint'
basepath = '/h/mulan0/analysis/LHS1140/'
directoryname = 'LDSS3C'
...
def loadrun(directory, inputs, wavefile):
    wavebin = np.load(directory+wavefile+'.npy')[()]
...
inputs = Inputs(jointdirectories, path)
print('loading joint cube')
subcube = np.load(path+'_subcube.npy')[()]
for n, night in enumerate(inputs.nightname):
...
def transmission(flag='absolute', model=True):
    ...
    plt.tight_layout()
    plt.savefig('/home/hdiamond/LHS1140/transmission_spectrum_models.png', dpi=900)
    plt.show()
...
def alldata():
    ...
    plt.xlabel('wavelength [microns]', fontsize=20)
    plt.tight_layout()
    plt.savefig('/home/hdiamond/LHS1140/transmission_spectra_alldata.png', dpi=900)
    plt.show()
...
def parameters():
    ...
    plt.savefig('/home/hdiamond/LHS1140/parameter_vs_wavelength.png', dpi=900)
    plt.show()
